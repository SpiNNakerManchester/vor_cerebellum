...
from vor_cerebellum.utilities import (
    ...
    generate_head_position_and_velocity, write_header, POS_TO_VEL,
...
target_reaching_type = args.experiment
...
cf_pc_connections = sim.Projection(
    ...
    label='cf_pc',  # receptor_type='COMPLEX_SPIKE',
...
write_header("Target reaching experiment type:" + target_reaching_type)
if target_reaching_type == "bistate":
...
elif target_reaching_type == "zero":
    head_pos[:] = 0.0
    head_vel[:] = 0.0
elif target_reaching_type == "constant":
    head_pos[:] = -0.5
    head_vel[:] = 0.0
elif target_reaching_type == "tristate":
    head_pos[:midway_point // 2] = -1.0
    head_pos[midway_point // 2:midway_point] = 0.0
    head_pos[midway_point:midway_point // 2 + midway_point] = 1.0
    head_pos[midway_point // 2 + midway_point:] = 0.0
    head_vel[:] = np.clip(
        np.diff(np.concatenate((head_pos, [head_pos[0]]))) / POS_TO_VEL,
        a_min=-1,
    ...
    raise AttributeError("Unrecognised target reaching experiment type:",
...
vn_to_icub = sim.Projection(VN_population, icub_vor_env_pop,
                            sim.AllToAllConnector(), label="VN-iCub")
